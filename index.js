      ...
      if (command === meme.name) {
        if (args.length <= meme.textBoxes.length) {
          ...
          message.channel.send(
            `maximum number of arguments for $${meme.name} is ${meme.textBoxes.length} `
          );
...
function verify(message, functionName, textBoxes) {
  ...
  message.channel.send(
    `Bep boop to use this command, type "$${functionName} *${formattedTextNames}*" Beeop bop`
  );
}
...
function attachText(img, context, text, format) {
  ...
  const xCoord = format.textPosition
    ? format.textPosition[0]
    : format.position[0];
  const yCoord = format.textPosition
    ? format.textPosition[1]
    : format.position[1];
